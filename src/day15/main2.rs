...
use crate::day15::main2::ParamMode::{READ,WRITE};
use crate::day15::main2::Direction::{N,E,S,W};
use crate::day13::main2::print_board;
...
pub fn play(program: &str) -> i64 {
    ...
    let (result, newstate) = explore(state);
    return explore2(newstate);
}

pub fn explore(state: ProgramState) -> (i64, ProgramState) {
    let loc = (0, 0);
    ...
    while queue.len() > 0 {
        ...
        for (loc_nb, dir) in neighbours {
            ...
            match output.0[0] {
                ...
                2 => {
                    //Target found!
                    return (past_distance + 1, state_clone);
                }
                _ => panic!()
            }
        }
...
pub fn explore2(state: ProgramState) -> (i64) {
    let loc = (0, 0);
    let mut queue: VecDeque<(i64, i64)> = VecDeque::new();
    let mut have_been: HashSet<(i64, i64)> = HashSet::new();
    let mut type_map: HashMap<(i64, i64), i64> = HashMap::new();
        ...
        for (loc_nb, dir) in neighbours {
            ...
            match output.0[0] {
                ...
                1 | 2 => {
                    ...
                    queue.push_front(loc_nb.clone());
                    program_map.insert(loc_nb.clone(), state_clone);
                    max_distance = max_distance.max(past_distance + 1);
                }
                _ => panic!()
            }
        }
