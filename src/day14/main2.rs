...
pub fn getFuelFromOre(input: &str, ore_have: i64) -> i64 {
    let mut min: i64 = 0;
    let mut max: i64 = 1000000000000;

    //Binary search
    while max > min {
        let mid = (min + max + 1) / 2;
        let ore_needed = findOreNeeded(input, mid);
        if ore_needed > ore_have {
            max = mid - 1;
        } else {
            min = mid;
        }
...
#[cfg(test)]
mod test {
    use crate::day14::main2::getFuelFromOre;
...
7 A, 1 D => 1 E
7 A, 1 E => 1 FUEL";
        let result = getFuelFromOre(input, 1000000000000);
        assert_eq!(result, 34482758620);
    }
