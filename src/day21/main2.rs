...
pub fn main(program: &str) -> i64 {
    ...
    let inputs: Vec<i64> = commands.chars().map(|c| c as i64).collect();
    ...
    println!("Halt: {}", output.1);
    return output.0[output.0.len() - 1];
}
