...
use crate::day17::main1::ParamMode::{READ,WRITE};

pub fn alignment_sum(program: &str) -> i64 {
    //Get input as string
    ...
    return alignment_sum_parsed(&parsed);
}

pub fn alignment_sum_parsed(parsed: &Vec<char>) -> i64 {
    ...
    for (point, val) in &map {
        if *val != '#' {continue}
        if neighbours(&map, *point).len() != 4 {continue}
        sum += point.0 * point.1;
    }
